...
class CategoryValidator:
    ...
    def __init__(self, api_key: str, model: str = "claude-sonnet-4-5-20250929", db_manager=None, csv_output_dir: Optional[Path] = None, max_concurrent: int = 50, rate_limit_delay: float = 0.1):
        """
        初始化分类校验器
        ...
        Args:
            ...
            rate_limit_delay: API调用间隔（秒，默认0.1秒）
        """
        ...
        async with self._concurrency_lock:
            if success:
                ...
                if self._consecutive_successes >= self._success_threshold:
                    ...
                    self._current_concurrent = min(self._current_concurrent + 5, self.max_concurrent)
                    if self._current_concurrent > old_concurrent:
                        self.logger.info(f"并发数增加: {old_concurrent} -> {self._current_concurrent}")
                    self._consecutive_successes = 0
            else:
                # 检查是否为服务端错误 (503, 504, Internal Server Error)
                is_server_error = any(err in error_msg for err in ['503', '504', 'Internal Server Error', '超时', 'timeout'])
                if is_server_error:
                    self._consecutive_successes = 0
                    old_concurrent = self._current_concurrent
                    # 降低并发数，最低为1
                    ...
                    if self._current_concurrent < old_concurrent:
                        self.logger.warning(f"检测到服务端错误，并发数降低: {old_concurrent} -> {self._current_concurrent}")
    ...
    async def validate_product_async(
        ...
        """
        self.logger.debug(f"验证产品分类: {product.asin}")
    ...
    async def validate_batch_async(
        ...
        self.logger.info(f"开始异步批量验证 {len(products)} 个产品 (初始并发数: 1, 最大并发数: {self.max_concurrent})")
        ...
        while pending_indices or active_tasks:
            ...
            while pending_indices and len(active_tasks) < self._current_concurrent:
                ...
                self.logger.info(f"进度: {idx + 1}/{len(products)} - {product.asin} (并发: {len(active_tasks) + 1}/{self._current_concurrent})")
            ...
            for task in done:
                ...
                try:
                    ...
                    if result is not None:
                        ...
                        self.logger.warning(f"产品 {products[idx].asin} 验证失败，不入库")
                except Exception as e:
                    self.logger.error(f"验证产品 {products[idx].asin} 时发生异常: {e}")
        ...
        self.logger.info(f"验证完成: 成功 {len(valid_results)}/{len(products)}, 失败 {failed_count}, "
    ...
    def validate_batch(
        ...
        for i, product in enumerate(products, 1):
            ...
            if result is not None:
                ...
                failed_count += 1
                self.logger.warning(f"产品 {product.asin} 验证失败，不入库")

        # 统计结果
        relevant_count = sum(1 for r in results if r.is_relevant)
        correct_count = sum(1 for r in results if r.category_is_correct)

        self.logger.info(f"验证完成: 成功 {len(results)}/{len(products)}, 失败 {failed_count}, "
...
"""
    ...
    def save_results_to_csv(
        ...
        """
        if not results:
            self.logger.warning("[Claude] 没有验证结果可保存")
            return None

        # 生成文件名
        if filename is None:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            filename = f"claude_validation_{keyword}_{timestamp}.csv"
        ...
        try:
            ...
            self.logger.info(f"[Claude] 验证结果已保存到CSV: {csv_path}")
            return csv_path

        except Exception as e:
            self.logger.error(f"[Claude] 保存CSV失败: {e}")
            return None
